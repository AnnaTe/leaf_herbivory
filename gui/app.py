...
class DesignerMainWindow(QtWidgets.QMainWindow, Ui_MainWindow, Data):
    ...
    * can be used on individual images or on images of a file directory
    ...
    def __init__(self, parent=None):
        ...
        self.pbDirectoryOpen_3.clicked.connect(self.select_dir_leaf)
        self.pbDirectoryExport_3.clicked.connect(self.select_dir_leaf)
        self.pbRun.clicked.connect(self.run_export)
        self.pbRun_3.clicked.connect(self.run_export_leaf)
    ...
    def select_dir_leaf(self):
        """opens directory selection dialog"""
        directory = QtWidgets.QFileDialog.getExistingDirectory(self, 'Select directory')
        if self.sender().text() == 'Export':
            self.lineEditDirOutLeaf.setText(directory)
        else:
            self.lineEditDirInLeaf.setText(directory)
            self.tabWidget.setCurrentIndex(1)
    ...
    def run_export_leaf(self):
        ...
        # create output directory if necessary
        outputdir = self.lineEditDirOutLeaf.text() + '/'
        os.makedirs(outputdir, exist_ok= True)
        ...
        for imagepath in paths:
            ...
            if self.cbMaskDir.isChecked() == True:
                ...
                outpath = folder + self.i.name + '_mask' + '.png'
                cv2.imwrite(outpath, self.i.mask)
                self.completed += 1
                self.statusbar.showMessage("Running: {} of {} image exported.".format(self.completed, self.total))
            
            if self.cbFilledDir.isChecked() == True:
                folder = outputdir + "filled/"
                os.makedirs(folder, exist_ok= True)
                outpath = folder + self.i.name + '_filled' + '.png'
                cv2.imwrite(outpath, self.i.filled)
                self.completed += 1
                self.statusbar.showMessage("Running: {} of {} image exported.".format(self.completed, self.total))
            
            if self.cbContourDir.isChecked() == True:
                folder = outputdir + "contour/"
                os.makedirs(folder, exist_ok= True)
                outpath = folder + self.i.name + '_contour' + '.png'
                cv2.imwrite(outpath, self.i.contours)
                self.completed += 1
                self.statusbar.showMessage("Running: {} of {} image exported.".format(self.completed, self.total))
            
            if self.cbValueDir.isChecked() == True:
                log = outputdir + "output.txt"
                with open(log, "a") as text_file:
                    text_file.write("{imagename}, {percvalue}\n".format(imagename = self.i.name, percvalue = round(value,2)))
